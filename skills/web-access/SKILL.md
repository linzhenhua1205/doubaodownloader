...
| 统一搜索路由（L1 专业站站内 → L2 Bing → L3 搜狗系兜底） | **search-router.py**（`scripts/search-router.py`：`--sources` 列表 / `--check` 可达性 / `--fetch-content <id>` 抓内容源；数据源注册表 SSOT 见 `scripts/config/source-registry.json`，新增/修改源只改 JSON） |
| 内容源直连抓取（AI 日报/云厂商文档/政策/市场研究） | **search-router.py --fetch-content**（ai-bot / maomu / idc / hothardware / geeksforgeeks / 21icsearch / techtarget / cloud-tencent / developer-aliyun / hhs-gov / electronics-tutorials；static 链接级、js SSR 标题摘要、web_fetch_only 用 WebFetch 工具、不可达自动跳过；完整清单见 `--content`） |
| URL 已知，需要从页面定向提取特定信息 | **WebFetch**（拉取网页内容，由小模型根据 prompt 提取，返回处理后结果） |
| URL 已知，需要原始 HTML 源码（meta、JSON-LD 等结构化字段） | **curl** |
| 知乎内容（搜索/热榜/问题/回答/文章/用户） | **zhihu-cli**（`~/.local/bin/zhihu`，官方 V4 API 结构化 JSON，需登录；见 `references/site-patterns/zhihu.com.md`） |
| 飞书文档（docx/wiki/docs）/ arXiv 论文源码 / 通用 PDF | **markdown-proxy**（`scripts/markdown-proxy/scripts/`：`fetch_feishu.py` 需 FEISHU_APP_ID/SECRET + docx 读权限 · `extract_tex.py` LaTeX 源码提取保公式/表格/文献 · `extract_pdf.sh` marker→pdftotext→pypdf 三级） |
| X/Twitter 单条推文 | **fetch-skill**（`scripts/fetch-skill/fetch.py`，FxTwitter API 零依赖；回复/时间线需 Camofox；见 `references/site-patterns/x.com.md`） |
| 微信公众号文章 `mp.weixin.qq.com/s/...`（单篇公开） | **wechat-claw**（`scripts/wechat-claw/read_wechat_article.py`，curl_cffi 指纹零依赖，纯文本正文最快）· **wechat-extractor**（`scripts/wechat-extractor/cli.js`，Node 富元数据：封面/biz/sn/mid/文章类型/17 种错误码诊断，需富元数据或诊断时用）；两者失败回退 wechat-article-fetch 三要素方案（微信UA+chksm+剥poc_token），仍失败才 CDP 兜底；见 `references/site-patterns/mp.weixin.qq.com.md`） |
| 公众号历史文章全量提取（含正文） | **wechat-claw crawler.py**（`scripts/wechat-claw/crawler.py`，需微信公众平台凭证 cookie+token 手工注入；**云端禁止自动登录**——机房 IP 风控封号，凭证仅存活几小时；见 site-patterns） |
| 非公开内容，或已知静态层无效的平台（小红书等公开内容也被反爬限制） | **浏览器 CDP**（直接，跳过静态层） |
...
## 访问方式查表（token 优先，每次联网前必查）

**访问任何网站前，先查表确定访问方式优先级**，按 token 从省到贵依次尝试，失败自动降级——避免每次盲目从最重的浏览器/CDP 开始试，也避免用最费 token 的方式抓最简单的页面。

```bash
# 查某 URL 的访问方式优先级链
python3 scripts/source-access-lookup.py --url "https://www.tomshardware.com/xxx"
# 按域名 / 源 id 查
python3 scripts/source-access-lookup.py --domain semianalysis.com
python3 scripts/source-access-lookup.py --id arxiv
# 全表（26 源） / 推荐首选
python3 scripts/source-access-lookup.py --list
python3 scripts/source-access-lookup.py --recommend servethehome
```

**优先级原则**（token 省 → 费，SSOT 见 `scripts/config/source-registry.json` 的 `access_chain` 字段）：

| 优先级 | 方式 | token 成本 | 适用 |
|:------:|:-----|:----------:|:-----|
| 1 | rss / api | 最低（结构化） | 有 RSS/API 的源（arXiv、SemiAnalysis archive、TechCrunch feed） |
| 2 | jina（r.jina.ai） | 低（Markdown） | 文章/博客/文档类正文页面；限 20 RPM |
| 3 | static | 中（HTML 提取） | 静态 HTML 可直连的源（curl/requests → 正文提取） |
| 4 | web_fetch | 中（工具提取） | 通用默认；小模型按 prompt 返回提取结果 |
| 5 | js | 高（渲染） | JS 渲染页面（21icsearch/techtarget 等） |
| 6 | browser / cdp | 最高 | 登录态、交互、反爬兜底（CSDN 521 等） |
| - | local | 零 | 本地子模块/离线数据 |

**降级纪律**：chain 首选项失败 → 依次降级下一级；全部失败才换源或升级 CDP。查表命中但访问异常（反爬/403/挂死）→ 更新注册表 `access_chain` 与 `reachable`（把经验沉淀回 SSOT），并记入 web-access-log 台账。

## 原始素材落盘（溯源证据，调研抓取必做）

**写入调研文件的每条关键信号，其原始抓取素材必须落盘**，形成三级证据链：日报 → 01_survey 调研文件 → tmp/raw 原始素材（+ web-access-log.csv URL 台账）。

```bash
# 约定：正文提取后的 Markdown 落盘（克制——只落「写进调研文件的关键文章」，防磁盘膨胀）
mkdir -p tmp/raw/<日期>/
# 文件命名: <日期>/<domain>-<slug>.md
```

- 抓取关键文章时（curl/web_fetch 拿到正文后）→ 正文整理为 `tmp/raw/<日期>/<domain>-<slug>.md`
- 调研文件（01_survey/...）中该条目来源标注追加 `📎 raw: tmp/raw/<日期>/<domain>-<slug>.md`
- 日报「调研跟踪摘要」领域节标注调研文件路径，关键动态带 raw 路径（消费方：weekly-report-generator）
- 原始 HTML（未提炼）不落盘——正文提取版已含溯源所需全部信息；确需原始 HTML（meta/JSON-LD）时用 curl 现场读取，用完即弃

## 访问记录（任务收尾必做）

**每次联网任务收尾时**，把本次任务访问过的 URL 批量追加到 Web 访问台账（`knowledge/weekly-reports/07_kb_stat/99_data/web-access-log.csv`）。这是「来源可追溯」的系统级实现——区别于文档里的参考来源（文档级），台账累积全局访问历史，供周报统计与数据源质量评估（source-registry 联动）消费。

```bash
# 批量追加（草稿文件：每行一条，tab 分隔 url\ttitle\tchannel\tpurpose\tstatus\tnote）
python3 scripts/tools/web-access-log.py --file /tmp/web_draft.tsv

# 单条追加
python3 scripts/tools/web-access-log.py --url "https://..." --title "标题" \
...
**规则**：
- **成功失败都记**——失败记录（status=fail）是数据源可达性实证的输入，与 source-registry.json 的质量评估联动
- **批量优先**：任务收尾一次性追加，不在每次访问间打断工作流
- **去重内置**：同 URL 30 天内自动跳过，重复追加无副作用
- 渠道取值：`web_fetch` / `web_search` / `search-router` / `curl` / `requests` / `browser` / `cdp` / `zhihu-cli` / `fetch-skill` / `wechat-claw` / `wechat-extractor` / `markdown-proxy` / `jina` / `other`
- 用途取值：调研 / 归档 / 核验 / 日报 / 周报 / 深度分析 / 其他
...
| `references/site-patterns/zhihu.com.md` | 目标为知乎内容（搜索/热榜/问题/回答/文章）时，zhihu-cli 用法与陷阱 |
| `references/site-patterns/x.com.md` | 目标为 X/Twitter 内容（单推文/回复/时间线/X Article）时，fetch-skill 用法与陷阱 |
| `references/site-patterns/mp.weixin.qq.com.md` | 目标为微信公众号文章（单篇/公众号全量）时，wechat-claw 用法与凭证规则 |
| `scripts/fetch-skill/fetch.py` | 通用抓取器（fetch-skill 分支）：普通网页/X 推文/微信文章统一路由 |
| `scripts/wechat-claw/read_wechat_article.py` | 微信单篇文章 curl_cffi 读取器（wechat-claw 分支，零依赖） |
| `scripts/wechat-claw/crawler.py` | 公众号全量提取器（wechat-claw 分支，需凭证；云端禁自动登录） |
| `scripts/wechat-extractor/cli.js` | 微信文章富元数据提取器（wechat-extractor 分支：封面/biz/sn/mid/类型/17 错误码；`--summary`/`--content`/`--md`） |
| `scripts/markdown-proxy/` | 通用 URL→Markdown 提取器（qiaomu v2.1.0）：飞书/arXiv LaTeX/PDF/代理级联 |
| `scripts/search-router.py` | 统一搜索路由：L1 专业站 → L2 Bing → L3 搜狗系；`--content`/`--fetch-content` 内容源抓取 |
| `scripts/config/source-registry.json` | 数据源注册表 SSOT：专业站/内容源/搜索引擎 + 可达性 + 访问方式（static/js/web_fetch_only） |
| `scripts/tools/web-access-log.py` | Web 访问台账：任务收尾批量追加 时间/URL/标题/渠道/用途/状态 到 07_kb_stat/99_data/web-access-log.csv；`--stats` 汇总 |
