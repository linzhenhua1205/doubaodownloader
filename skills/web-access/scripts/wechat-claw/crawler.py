...
def save_credentials(cookie, token, path="credentials.json"):
    ...
    os.chmod(path, 0o600)  # 凭证文件仅属主可读写 (安全加固)
    print(f"[✓] 凭证已保存到 {path} (权限 600)")
...
def get_credentials_smart(headless=False):
    ...
    print("=" * 50)
    print("你想如何提供登录凭证？")
    print("1. [按回车] -> 自动启动浏览器扫码获取 (⚠️ Playwright 已禁用 2026-08-12，此选项不可用)")
    print("2. [粘贴 JSON] -> 贴入手工抓包获取的 {\"cookie\":\"...\",\"token\":\"...\"} 文本 (推荐云服务器使用)")
    print("=" * 50)

    raw = input("> ").strip()
    if not raw:
        print("[✗] Playwright 已禁用，无法自动扫码。请选择方式 2 手工粘贴 JSON 凭证。")
        sys.exit(1)
