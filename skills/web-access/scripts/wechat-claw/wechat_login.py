...
# ⚠️ Playwright 默认禁用 (2026-08-12)：无头服务器环境启动浏览器会导致系统挂死
# 如需启用：export WECHAT_PLAYWRIGHT_ENABLED=1
_PLAYWRIGHT_ENABLED = os.environ.get("WECHAT_PLAYWRIGHT_ENABLED") == "1"
...
def playwright_login(headless=False):
    ...
    if not _PLAYWRIGHT_ENABLED:
        print("[✗] Playwright 已禁用（WECHAT_PLAYWRIGHT_ENABLED=1 可启用）")
        print("    请改用手工注入凭证方式：crawler.py --credentials '{\"cookie\":\"...\",\"token\":\"...\"}'")
        return None, None
