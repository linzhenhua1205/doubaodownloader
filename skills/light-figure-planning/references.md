...
- 入口：`python scripts/generate_schematic.py "描述" -o out.png --doc-type journal --iterations 2 -v`；Python API `ScientificSchematicGenerator.generate_iterative()`。  # ⚠️ 设计承诺（示意图生成（语义判断），当前由 LLM 直接产出）
