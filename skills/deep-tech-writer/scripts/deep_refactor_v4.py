#!/usr/bin/env python3
"""
深度重构markdown文件 v4 - 优化版

修复v3问题：
1. 补充更多emoji字符
2. 改进关键词提取（避免完整标题作为关键词）
3. 改进概要生成（确保是一句话总结）
4. 修复四级标题降级为三级
5. 确保所有章节标题无emoji
"""
...
def extract_implicit_sections(content_text):
    ...
    # emoji正则模式（用字符类）
    emoji_pattern = '[' + re.escape(EMOJI_CHARS) + ']'
    ...
    for line in lines:
        ...
        # 模式1: emoji + **标题**
        m = re.match(r'^' + emoji_pattern + r'\s*\*\*(.+?)\*\*\s*$', stripped)
        if m:
            is_title = True
            title_text = clean_title(m.group(1))
        
        # 模式2: emoji + 短标题（<25字，非句子结尾，非列表项）
        if not is_title:
            m = re.match(r'^' + emoji_pattern + r'\s+(.+)$', stripped)
            if m:
...
def generate_summary_keywords(title, sections):
    ...
    for part in parts:
        part = part.strip()
        # 过滤条件：长度2-12字，不是停用词，不是纯数字，不是完整标题
    ...
    if all_paragraphs:
        ...
        if period_idx > 10 and period_idx <= 100:
        ...
        # 没有找到段落，用标题构造
        title_short = clean_title(title)
        summary = f'本文深入分析{title_short}的核心内容，探讨相关技术要点与行业影响。'
        if len(summary) > 100:
            summary = summary[:97] + '...'
    ...
    if len(summary) < 15:
        title_short = clean_title(title)
        summary = f'本文围绕{title_short}展开深入分析，涵盖核心内容与关键要点。'
        if len(summary) > 100:
            summary = summary[:97] + '...'
...
def main():
    if len(sys.argv) < 2:
        print('用法: python3 deep_refactor_v4.py <目录路径>')
        sys.exit(1)
