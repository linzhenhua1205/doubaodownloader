...
def process_file(filepath):
    ...
    try:
        ...
        if remove_ranges:
            cl_count = sum(1 for _, _, t in section_ranges if is_changelog_title(t) and (_, _) in [(r[0], r[1]) for r in remove_ranges])
            cl_count = 0
            ref_count = 0
        ...
        for e in all_entries:
            ...
            if v:
                had_table_format = True

        if clean_entries:
            stats["history_preserved"] = len(clean_entries)
            if not had_table_format:
        ...
        if step2_clean.endswith("---"):
            step2_clean = step2_clean[:-3].rstrip()
