#!/usr/bin/env python3
"""
深度重构数据库与存储目录下的markdown文档
遵循deep-tech-writer六步工作流进行深度重构
"""
...
def remove_emoji_from_title(title):
    """去除标题中的emoji前缀"""
    emoji_pattern = r'^[\U0001F300-\U0001F9FF\U00002600-\U000027BF\U0001F600-\U0001F64F\U0001F680-\U0001F6FF\s]+'
    return re.sub(emoji_pattern, '', title).strip()


def clean_section_title(title):
    """清理章节标题：去除emoji、编号等"""
    title = remove_emoji_from_title(title)
...
def extract_content_section_body(content):
    """提取"内容"章节下的正文内容，处理三级标题提升为二级标题"""
    pattern = r'^##\s*(?:.*?)内容.*?$\n(.*?)(?=\n##\s|\Z)'
...
def extract_meaningful_content(content):
    ...
    content_body = extract_content_section_body(body)
    ...
    for section in sections:
        if is_junk_section(section['title']):
            continue
        if '目录' in section['clean_title']:
            continue
        if '参考文件' in section['clean_title'] or 'Changelog' in section['clean_title']:
            continue
        
        section_body = re.sub(r'^##\s+.+?\n', '', section['content'], count=1).strip()
        
        if not section_body:
            continue
        
        if 'pgvector插件让PostgreSQL同时成为关系型数据库和向量数据库' in section_body:
            continue
        if '向量数据库的兴衰故事' in section_body:
            continue
        if 'PostgreSQL作为开源关系型数据库的标杆' in section_body:
            continue
        
        clean_title = clean_section_title(section['title'])
        meaningful_parts.append(f'## {clean_title}\n\n{section_body}')
    ...
    return body_clean if len(body_clean) > 200 else None
...
def find_internal_references(title, content):
    ...
    if '存储' in title or 'NVMe' in title or 'SSD' in title:
...
def generate_toc(main_sections):
    """生成目录（只包含核心二级标题）"""
    toc_lines = []
    for section_title in main_sections:
        slug = re.sub(r'[^\w\u4e00-\u9fff-]', '-', section_title).lower()
        slug = re.sub(r'-+', '-', slug).strip('-')
        toc_lines.append(f'- [{section_title}](#{slug})')
    return '\n'.join(toc_lines)


def extract_main_sections(content_body):
    """从正文中提取二级标题作为目录"""
    pattern = r'^##\s+(.+?)\s*$'
    matches = re.findall(pattern, content_body, re.MULTILINE)
...
def refactor_document(filepath):
    ...
    try:
        ...
        if not meaningful_content or len(meaningful_content) < 100:
            return False, '无有效内容可提取'
        
        summary, keywords = generate_summary_and_keywords(title, content)
        
        main_sections = extract_main_sections(meaningful_content)
        
        toc = generate_toc(main_sections) if main_sections else ''
...
def main():
    print("=" * 70)
    print("深度重构数据库与存储目录下的markdown文档")
    ...
    for i, filepath in enumerate(md_files, 1):
        ...
        if success:
            ...
            print(f"         ✅ 完成 | {result['sections_count']}个章节 | {result['keywords_count']}个关键词")
        else:
            fail_count += 1
    ...
    quality_script = BASE_DIR / "skills" / "deep-tech-writer" / "scripts" / "check_tech_doc_quality.py"
    if quality_script.exists():
        print("🔍 运行质量自检...")
        os.system(f'python "{quality_script}" "{TARGET_DIR}" --report')
