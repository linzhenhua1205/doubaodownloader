#!/usr/bin/env python3
"""
深度重构markdown文件 v6 - 概要质量最终优化版

修复v5问题：
1. 优化列表型内容的概要生成（17个模板化概要）
2. 从列表项中提取信息组合成有意义的概要
3. 更好地处理表格、新闻类、资讯类文章
"""
...
def generate_summary_from_lists(title, sections):
    ...
    if first_section_items:
        items_text = '、'.join(first_section_items[:3])
        if len(items_text) > 20:
            summary = f'{title_clean}：{items_text}等内容。'
            if len(summary) <= 100:
                return summary
    ...
    if all_list_items:
        ...
        for item in all_list_items:
            if len(item) >= 15 and len(item) <= 80:
                summary = f'{title_clean}：{item}'
    ...
    if sec_titles:
        ...
        summary = f'本文从{secs_text}等方面分析{title_clean}。'
        if len(summary) <= 100:
            return summary
    
    # 策略4：精简标题+核心信息
    if len(title_clean) <= 20:
        summary = f'本文系统梳理{title_clean}的核心内容与关键信息。'
    else:
        short_title = title_clean[:20]
        summary = f'本文系统梳理{short_title}...的核心内容与关键信息。'
...
def main():
    if len(sys.argv) < 2:
        print('用法: python3 deep_refactor_v6.py <目录路径>')
        sys.exit(1)
    ...
    report_path = os.path.join(target_dir, '_refactor_report_v6.json')
