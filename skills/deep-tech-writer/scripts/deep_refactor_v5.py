#!/usr/bin/env python3
"""
深度重构markdown文件 v5 - 最终优化版

修复v4问题：
1. 修复emoji+变体选择符的标题识别问题
2. 改进概要生成（跳过列表，找真正的段落）
3. 清理正文中残留的分隔线
4. 更好的关键词和概要质量
"""
...
def is_emoji_char(c):
    """判断是否为emoji字符（包括常见emoji范围）"""
    code = ord(c)
    # 常见emoji Unicode范围
    ranges = [
        (0x1F300, 0x1FAFF),  # 主要emoji
        (0x2600, 0x27BF),    # 符号与装饰
        (0x1F600, 0x1F64F),  # 表情
        (0x1F680, 0x1F6FF),  # 交通与设施
...
def generate_summary_keywords(title, sections):
    ...
    for _, content in sections:
        ...
        for p in paras:
            ...
            # 必须是完整段落（不是列表，长度足够）
            if len(p_clean) >= 30 and not p_clean.startswith(('- ', '* ', '1.', '2.', '3.', '4.', '5.')):
    ...
    if all_paragraphs:
        ...
        if len(title_short) > 20:
            # 截取标题前半部分
            summary = f'本文围绕{title_short[:15]}...展开深入分析，涵盖核心内容与关键要点。'
        else:
...
def main():
    if len(sys.argv) < 2:
        print('用法: python3 deep_refactor_v5.py <目录路径>')
        sys.exit(1)
    ...
    report_path = os.path.join(target_dir, '_refactor_report_v5.json')
