...
def generate_summary(body, title):
    ...
    # 策略1: 找"背景与意义"、"执行摘要"、"概述"等章节
    priority_sections = [
        r'##\s*(?:🌐\s*)?(?:背景与意义|背景介绍|文章背景|研究背景|项目背景)',
        r'##\s*(?:📋\s*)?(?:执行摘要|内容摘要|核心摘要|报告摘要)',
        r'##\s*(?:📖\s*)?(?:概述|简介|前言|引言)',
        r'##\s*(?:📊\s*)?(?:市场规模|行业现状|发展现状)',
    ]
    ...
    for pattern in priority_sections:
        ...
        if match:
            ...
            paragraphs = [p.strip() for p in section_text.split('\n\n') 
                         ...
                         and len(p.strip()) > 40]
            if paragraphs:
                candidate_paragraphs.extend(paragraphs)
                break
    ...
    if not candidate_paragraphs:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith('#'):
                ...
                if any(kw in clean_title for kw in ['快速导读', '核心要点', '目录', '内容提要', '摘要', '关键词']):
            ...
            # 跳过元信息引用块
            if re.match(r'^>\s*[\U0001F4C5\U0001F3F7\U0001F517\U0001F4DD\u2b50]', stripped) or stripped.startswith('> **'):
                continue
            ...
            if len(stripped) > 50 and not stripped.startswith(('-', '*', '•', '+')):
                candidate_paragraphs.append(stripped)
    ...
    if not candidate_paragraphs:
        ...
        if core_match:
            ...
            bullets = re.findall(r'[-*•]\s*\**([^*]+)\**[：:]?', core_text)
            if not bullets:
                bullets = re.findall(r'[-*•]\s+(.+?)(?=\n|$)', core_text)
            if bullets:
                first_bullet = bullets[0].strip()
                if len(first_bullet) > 20:
                    candidate_paragraphs.append(first_bullet)
    
    # 选择最好的段落
    best_summary = ""
    
    if candidate_paragraphs:
        # 优先选择包含"市场规模"、"产业"、"技术"等关键词的
        scored = []
        for p in candidate_paragraphs:
            score = len(p)
            if any(kw in p for kw in ['市场规模', '产业', '行业', '技术', '报告', '研究']):
                score += 50
            if any(kw in p for kw in ['亿元', '亿美元', '万亿', '%', '同比']):
                score += 30
            scored.append((p, score))
        
        scored.sort(key=lambda x: x[1], reverse=True)
        best_summary = scored[0][0]
    else:
        best_summary = f"本文围绕{title}主题展开深入分析，涵盖技术原理、应用实践与发展趋势等核心内容。"
    
    # 清理并格式化
    summary = clean_summary_text(best_summary)
...
def normalize_keyword(kw):
    ...
    return normalize_map.get(kw, kw)
...
def generate_keywords(body, title, fm):
    ...
    def add_keyword(kw, source='unknown', priority=0):
        kw = normalize_keyword(kw.strip())
        if kw and kw not in seen and is_good_keyword(kw):
            candidates.append((kw, source, priority))
            seen.add(kw)
    ...
    tech_in_title = re.findall(
        ...
        r'算力|Token|AI编程|代码生成|医疗AI|AI电商|具身智能)',
        title_clean
    )
    for kw in tech_in_title:
        add_keyword(kw, 'title_tech', 100)
    ...
    for pattern, kw in domain_patterns:
        if re.search(pattern, title_clean):
            add_keyword(kw, 'title_domain', 90)
    
    # 3. 从frontmatter的tags提取（高优先级）
    ...
    if tags_match:
        ...
        for t in tag_items:
            t = t.strip()
            if t and t != 'null':
                add_keyword(t, 'fm_tags', 80)
    
    # 4. 从正文提取高频技术关键词（中优先级）
    tech_kw_freq = {}
    
    tech_patterns = [
        (r'大模型|大语言模型|LLM', 5, '大模型'),
        (r'GPT(?:-\d+)?', 4, 'GPT'),
        (r'Claude', 4, 'Claude'),
        (r'Gemini', 4, 'Gemini'),
        (r'Llama|Llama', 4, 'Llama'),
    ...
    for pattern, weight, normalized_kw in tech_patterns:
        count = len(re.findall(pattern, body))
        if count > 0:
            score = count * weight
            if normalized_kw in tech_kw_freq:
                tech_kw_freq[normalized_kw] += score
            else:
                tech_kw_freq[normalized_kw] = score
    ...
    for kw, score in sorted_tech:
        if score >= 15:
            add_keyword(kw, 'body_tech', 70 + min(score, 30))
    
    # 5. 从标题提取其他有意义的词汇（低优先级）
    title_parts = re.split(r'[：:、，,\s与及和]+', title_main)
    for part in title_parts:
        part = part.strip()
        if is_good_keyword(part) and len(part) >= 3:
            add_keyword(part, 'title', 50)
    ...
    if len(final_keywords) < 3:
        ...
        for kw in backups:
            if kw not in final_keywords and len(final_keywords) < 5:
                final_keywords.append(kw)
...
def main():
    if len(sys.argv) < 2:
        print('用法: python3 deep_refactor_ai_docs_v4.py <目录路径>')
        sys.exit(1)
    ...
    report_path = os.path.join(target_dir, '_refactor_report_v4.json')
