...
> **概要**: 计算系统命名艺术，解析名称、动作、关系、版本、来源与状态六大命名模式
>
> **关键词**: 命名艺术 · 命名模式 · 接口命名 · 版本化 · 状态命名
...
- [1. 引言](#1-引言)
  - [1.1 命名为什么是计算机科学中最难的事](#11-命名为什么是计算机科学中最难的事)
  - [1.2 六大模式的分类框架](#12-六大模式的分类框架)
- [2. 六大命名模式深度解析](#2-六大命名模式深度解析)
  - [2.1 名称型](#21-名称型)
    - [2.1.1 典型实例](#211-典型实例)
    - [2.1.2 名称型的适用条件](#212-名称型的适用条件)
    - [2.1.3 权衡：为什么 `ioctl` 是个坏命名](#213-权衡为什么-ioctl-是个坏命名)
  - [2.2 动作+对象型](#22-动作对象型)
    - [2.2.1 典型实例](#221-典型实例)
    - [2.2.2 动宾结构的方向选择](#222-动宾结构的方向选择)
    - [2.2.3 动词选择的艺术](#223-动词选择的艺术)
  - [2.3 体现关系型](#23-体现关系型)
    - [2.3.1 Go 的 Interface 命名约定](#231-go-的-interface-命名约定)
    - [2.3.2 Java 的 Interface 命名体系](#232-java-的-interface-命名体系)
    - [2.3.3 Linux 内核的 `_ops` 模式](#233-linux-内核的-_ops-模式)
    - [2.3.4 C++ Concepts 的命名](#234-c-concepts-的命名)
    - [2.3.5 Rust Trait 命名](#235-rust-trait-命名)
    - [2.3.6 体现关系型的核心原则](#236-体现关系型的核心原则)
  - [2.4 版本/后缀型](#24-版本后缀型)
    - [2.4.1 Windows 的 Ex 后缀](#241-windows-的-ex-后缀)
    - [2.4.2 Linux 的版本数字命名](#242-linux-的版本数字命名)
    - [2.4.3 版本化的三种策略](#243-版本化的三种策略)
    - [2.4.4 API 版本化的三种模式](#244-api-版本化的三种模式)
  - [2.5 来源标识型](#25-来源标识型)
    - [2.5.1 Rust 的 From/Into 体系](#251-rust-的-frominto-体系)
    - [2.5.2 Go 的转换命名](#252-go-的转换命名)
    - [2.5.3 三种转换命名范式](#253-三种转换命名范式)
  - [2.6 状态型](#26-状态型)
    - [2.6.1 谓词助动词的选择](#261-谓词助动词的选择)
    - [2.6.2 Linux 内核状态命名](#262-linux-内核状态命名)
    - [2.6.3 Systemd 单元状态的命名](#263-systemd-单元状态的命名)
    - [2.6.4 Kubernetes 对象状态命名](#264-kubernetes-对象状态命名)
    - [2.6.5 TCP 连接状态命名](#265-tcp-连接状态命名)
  - [2.7 有序序列型——时间与序号的秩序表达](#27-有序序列型时间与序号的秩序表达)
    - [2.7.1 日期版本（CalVer）——时间直接作为版本名](#271-日期版本calver时间直接作为版本名)
    - [2.7.2 时序 ID——有序序列的直接编码](#272-时序-id有序序列的直接编码)
    - [2.7.3 顺序号在版本中的层级表达](#273-顺序号在版本中的层级表达)
    - [2.7.4 时间戳日志命名——最广泛的时序标识](#274-时间戳日志命名最广泛的时序标识)
    - [2.7.5 构建/制品时序编码](#275-构建制品时序编码)
    - [2.7.6 分布式系统中的逻辑时钟命名](#276-分布式系统中的逻辑时钟命名)
    - [2.7.7 网络协议中的序列号命名](#277-网络协议中的序列号命名)
    - [2.7.8 数据库的日志序列号（LSN）命名](#278-数据库的日志序列号lsn命名)
    - [2.7.9 有序序列的核心权衡](#279-有序序列的核心权衡)
- [3. 优秀工程的命名体系深度剖析](#3-优秀工程的命名体系深度剖析)
  - [3.1 Linux 内核命名体系](#31-linux-内核命名体系)
    - [3.1.1 系统调用命名方案](#311-系统调用命名方案)
    - [3.1.2 驱动命名统一性](#312-驱动命名统一性)
    - [3.1.3 ioctl 号编码方案](#313-ioctl-号编码方案)
    - [3.1.4 设备树命名](#314-设备树命名)
  - [3.2 Windows API 命名体系](#32-windows-api-命名体系)
    - [3.2.1 匈牙利记法](#321-匈牙利记法)
    - [3.2.2 A/W 后缀方案](#322-aw-后缀方案)
    - [3.2.3 句柄体系](#323-句柄体系)
  - [3.3 Git 命名设计哲学](#33-git-命名设计哲学)
    - [3.3.1 动词命令体系](#331-动词命令体系)
    - [3.3.2 Git 的内部对象命名](#332-git-的内部对象命名)
    - [3.3.3 ref 命名规范](#333-ref-命名规范)
  - [3.4 Go 标准库命名哲学](#34-go-标准库命名哲学)
    - [3.4.1 极简主义命名](#341-极简主义命名)
    - [3.4.2 包命名约定](#342-包命名约定)
    - [3.4.3 接口单方法命名](#343-接口单方法命名)
  - [3.5 Rust 命名约定](#35-rust-命名约定)
    - [3.5.1 层级命名规范](#351-层级命名规范)
    - [3.5.2 构造函数命名](#352-构造函数命名)
  - [3.6 Systemd 命名体系](#36-systemd-命名体系)
    - [3.6.1 单元命名的分层](#361-单元命名的分层)
    - [3.6.2 配置键命名](#362-配置键命名)
  - [3.7 Kubernetes 命名体系](#37-kubernetes-命名体系)
    - [3.7.1 资源对象命名](#371-资源对象命名)
    - [3.7.2 labels/annotations 命名](#372-labelsannotations-命名)
- [4. 命名冲突与解决策略](#4-命名冲突与解决策略)
  - [4.1 命名冲突的来源](#41-命名冲突的来源)
  - [4.2 解决策略](#42-解决策略)
- [5. 命名一致性的深层价值](#5-命名一致性的深层价值)
  - [5.1 一致性降低认知负载](#51-一致性降低认知负载)
  - [5.2 一致性提供的"模式感"](#52-一致性提供的模式感)
  - [5.3 一致性的代价](#53-一致性的代价)
- [6. 命名的认知心理学](#6-命名的认知心理学)
  - [6.1 数字启发式](#61-数字启发式)
  - [6.2 首字母效应](#62-首字母效应)
  - [6.3 对称性满足](#63-对称性满足)
  - [6.4 语境效应](#64-语境效应)
- [7. 总结：命名原则 50 条](#7-总结命名原则-50-条)
  - [7.1 原则总表](#71-原则总表)
...
- [8. 命名中的关系体现——隐式/显式/动作顺序/对象先后](#8-命名中的关系体现隐式显式动作顺序对象先后)
  ...
  - [8.2 隐式关系——不出现在命名中的关系](#82-隐式关系不出现在命名中的关系)
    - [8.2.1 通过上下文推断的关系](#821-通过上下文推断的关系)
    - [8.2.2 通过文档/注释声明的关系](#822-通过文档注释声明的关系)
  - [8.3 结构关系——通过代码/目录结构暗示的关系](#83-结构关系通过代码目录结构暗示的关系)
    - [8.3.1 目录/包结构中的包含关系](#831-目录包结构中的包含关系)
    - [8.3.2 文件内成员关系](#832-文件内成员关系)
    - [8.3.3 类型 vs 实例的关系](#833-类型-vs-实例的关系)
    - [8.3.4 文件命名中的模块边界关系](#834-文件命名中的模块边界关系)
  - [8.4 可解析关系——通过命名规则反向推导的关系](#84-可解析关系通过命名规则反向推导的关系)
    - [8.4.1 后缀约定的关系推导](#841-后缀约定的关系推导)
    - [8.4.2 `is_`/`has_`/`can_` 前缀的关系推导](#842-is_has_can_-前缀的关系推导)
    - [8.4.3 引用/指针命名的关系推导](#843-引用指针命名的关系推导)
    - [8.4.4 错误处理中的关系链](#844-错误处理中的关系链)
  - [8.5 显式关系——直接在命名中编码的关系](#85-显式关系直接在命名中编码的关系)
    - [8.5.1 动作+对象的关系——最基本的显式关系](#851-动作对象的关系最基本的显式关系)
    - [8.5.2 两个对象的先后/主客关系](#852-两个对象的先后主客关系)
    - [8.5.3 接收器与参数的「主客关系」](#853-接收器与参数的主客关系)
  - [8.6 容器-被包含关系——层次结构与所属关系](#86-容器-被包含关系层次结构与所属关系)
    - [8.6.1 路径分隔符表达的层次关系](#861-路径分隔符表达的层次关系)
    - [8.6.2 层级命名中的方向性](#862-层级命名中的方向性)
    - [8.6.3 上下文管理器中的 `with` 关系](#863-上下文管理器中的-with-关系)
    - [8.6.4 树/图结构中的节点关系](#864-树图结构中的节点关系)
  - [8.7 关系的变化与演变——重构中的关系维护](#87-关系的变化与演变重构中的关系维护)
    - [8.7.1 关系漂移——名字没变但实际关系变了](#871-关系漂移名字没变但实际关系变了)
    - [8.7.2 关系修复——通过改名纠正关系编码](#872-关系修复通过改名纠正关系编码)
    - [8.7.3 关系控制的三种策略](#873-关系控制的三种策略)
  - [8.8 关系的认知负担——什么情况下关系需要显式化](#88-关系的认知负担什么情况下关系需要显式化)
    - [8.8.1 关系需要显式化的信号](#881-关系需要显式化的信号)
    - [8.8.2 关系不应显式化的场景](#882-关系不应显式化的场景)
    - [8.8.3 关系显式化的代价](#883-关系显式化的代价)
  - [8.9 关系呈现的对比案例——好与坏](#89-关系呈现的对比案例好与坏)
    - [8.9.1 好的关系呈现](#891-好的关系呈现)
    - [8.9.2 差的关系呈现](#892-差的关系呈现)
    - [8.9.3 关系呈现的量化评估矩阵](#893-关系呈现的量化评估矩阵)
  - [8.10 关系呈现的核心原则](#810-关系呈现的核心原则)
    - [8.10.1 原则总表（N36-N50）](#8101-原则总表n36-n50)
    - [8.10.2 关系呈现方式的选择流](#8102-关系呈现方式的选择流)
    - [8.10.3 关系呈现的最终心法](#8103-关系呈现的最终心法)
- [参考文献](#参考文献)
- [参考文件](#参考文件)
- [Changelog](#changelog)
