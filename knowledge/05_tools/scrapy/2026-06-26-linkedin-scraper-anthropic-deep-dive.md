...
> **概要**: LinkedIn爬虫参考实现，以Anthropic人才分析用例驱动的深度设计
>
> **关键词**: LinkedIn爬虫 · 人才分析 · Anthropic · 数据抽取 · 反检测
...
- [1. 设计方法论：数据需求驱动架构](#1-设计方法论数据需求驱动架构)
  - [1.1 从分析目标倒推工程需求](#11-从分析目标倒推工程需求)
  - [1.2 工程约束矩阵](#12-工程约束矩阵)
  - [1.3 数据质量目标](#13-数据质量目标)
- [2. 人才发现引擎](#2-人才发现引擎)
  - [2.1 从数据需求推导发现策略](#21-从数据需求推导发现策略)
  - [2.2 路径 A：LinkedIn 搜索（核心路径）](#22-路径-alinkedin-搜索核心路径)
    - [2.2.1 工程岗位关键词体系](#221-工程岗位关键词体系)
    - [2.2.2 搜索策略](#222-搜索策略)
  - [2.3 路径 B：Google Dork（低风险初始扫描）](#23-路径-bgoogle-dork低风险初始扫描)
    - [2.3.1 Anthropic 场景的 Dork 策略](#231-anthropic-场景的-dork-策略)
  - [2.4 路径 C：Voyager API（高效详情获取）](#24-路径-cvoyager-api高效详情获取)
    - [2.4.1 为什么 API 适合详情获取而非发现](#241-为什么-api-适合详情获取而非发现)
  - [2.5 三路调度器（含 Anthropic 场景参数）](#25-三路调度器含-anthropic-场景参数)
- [3. 个人资料抽取器](#3-个人资料抽取器)
  - [3.1 设计原则：从分析精度反推动作](#31-设计原则从分析精度反推动作)
  - [3.2 DOM 解析 + 工作经历提取](#32-dom-解析-工作经历提取)
  - [3.3 技能过滤引擎（与 Anthropic Top 20 对齐）](#33-技能过滤引擎与-anthropic-top-20-对齐)
- [4. 数据分析管道](#4-数据分析管道)
  - [4.1 数据流架构](#41-数据流架构)
  - [4.2 完整 Pipeline 实现](#42-完整-pipeline-实现)
  - [4.3 可视化输出](#43-可视化输出)
- [5. 完整 Pipeline 编排](#5-完整-pipeline-编排)
- [6. 风险研判与边界条件](#6-风险研判与边界条件)
  - [6.1 数据质量风险](#61-数据质量风险)
  - [6.2 样本偏差风险](#62-样本偏差风险)
  - [6.3 与 Milan Griffes 原始分析的对标问题](#63-与-milan-griffes-原始分析的对标问题)
  - [6.4 对 AI 产业链的启示（反哺设计）](#64-对-ai-产业链的启示反哺设计)
- [关联知识](#关联知识)
- [参考文件](#参考文件)
- [Changelog](#changelog)
...
- `tools/2026-06-26-linkedin-scraper-architecture.md` — 通用架构（3608 行全量实现）
- `enterprise-mgmt/2026-06-17-ai-talent-strategy-anthropic.md` — Anthropic 人才分析原始报告
- `02_rd/03_hardware/08_storage/2026-06-17-diagnostic-projects-survey.md` — 数据质量验证方法论
