...
- [第1章 总论：芯片初始化的共性框架与恢复层次](#第1章-总论芯片初始化的共性框架与恢复层次)
  ...
  - [1.3 电源域与复位层次的芯片设计原理](#13-电源域与复位层次的芯片设计原理)
    - [1.3.1 电源域划分的芯片设计原则](#131-电源域划分的芯片设计原则)
    - [1.3.2 复位层次](#132-复位层次)
...
- [第2章 CPU 初始化与故障恢复](#第2章-cpu-初始化与故障恢复)
  - [2.1 CPU 供电初始化原理](#21-cpu-供电初始化原理)
    - [2.1.1 CPU 供电架构](#211-cpu-供电架构)
    - [2.1.2 CPU 上电时序原理](#212-cpu-上电时序原理)
  - [2.2 固件启动流程：从 Reset Vector 到 UEFI](#22-固件启动流程从-reset-vector-到-uefi)
    - [2.2.1 CPU 复位后第一条指令](#221-cpu-复位后第一条指令)
    - [2.2.2 微代码 (Microcode) 加载原理](#222-微代码-microcode-加载原理)
    - [2.2.3 Cache-As-RAM (CAR) 原理](#223-cache-as-ram-car-原理)
  - [2.3 内存子系统初始化（Memory Training）](#23-内存子系统初始化memory-training)
    - [2.3.1 DDR5 内存初始化的层次](#231-ddr5-内存初始化的层次)
    - [2.3.2 Write Leveling 的物理原理](#232-write-leveling-的物理原理)
    - [2.3.3 DDR5 内存训练失败的处理](#233-ddr5-内存训练失败的处理)
  - [2.4 PCIe 总线初始化与枚举](#24-pcie-总线初始化与枚举)
    - [2.4.1 PCIe 枚举原理](#241-pcie-枚举原理)
    - [2.4.2 资源分配中的 BAR 冲突处理](#242-资源分配中的-bar-冲突处理)
  - [2.5 监控子系统：PECI / MCA / CMCI](#25-监控子系统peci-mca-cmci)
    - [2.5.1 PECI (Platform Environment Control Interface)](#251-peci-platform-environment-control-interface)
    - [2.5.2 MCA (Machine Check Architecture)](#252-mca-machine-check-architecture)
    - [2.5.3 CMCI (Corrected Machine Check Interrupt)](#253-cmci-corrected-machine-check-interrupt)
  - [2.6 CPU 异常检测与故障恢复](#26-cpu-异常检测与故障恢复)
    - [2.6.1 CPU 故障类型与检测方法](#261-cpu-故障类型与检测方法)
    - [2.6.2 CPU 恢复机制层级](#262-cpu-恢复机制层级)
- [第3章 PCIe Switch 初始化与故障恢复](#第3章-pcie-switch-初始化与故障恢复)
  - [3.1 供电与时钟域](#31-供电与时钟域)
  - [3.2 固件初始化与配置空间填充](#32-固件初始化与配置空间填充)
    - [3.2.1 Switch 固件加载](#321-switch-固件加载)
    - [3.2.2 配置空间初始化](#322-配置空间初始化)
  ...
  - [3.5 PCIe Switch 故障恢复](#35-pcie-switch-故障恢复)
    - [3.5.1 链路级恢复](#351-链路级恢复)
    - [3.5.2 DPC (Downstream Port Containment)](#352-dpc-downstream-port-containment)
    - [3.5.3 端口重映射 (Port Remapping)](#353-端口重映射-port-remapping)
    - [3.5.4 全芯片恢复](#354-全芯片恢复)
- [第4章 USB 控制器初始化与故障恢复](#第4章-usb-控制器初始化与故障恢复)
  - [4.1 供电与时钟](#41-供电与时钟)
  - [4.2 固件初始化](#42-固件初始化)
    - [4.2.1 xHCI (eXtensible Host Controller Interface) 初始化](#421-xhci-extensible-host-controller-interface-初始化)
  - [4.3 USB PHY 初始化与校准](#43-usb-phy-初始化与校准)
  - [4.4 链路训练与枚举：USB 2.0 / 3.x / USB4](#44-链路训练与枚举usb-20-3x-usb4)
    - [4.4.1 USB 3.0 链路训练](#441-usb-30-链路训练)
    - [4.4.2 USB 2.0 高速检测握手 (Chirp)](#442-usb-20-高速检测握手-chirp)
  - [4.5 USB 异常检测与恢复](#45-usb-异常检测与恢复)
    - [4.5.1 USB 错误类型](#451-usb-错误类型)
    - [4.5.2 USB 多层次恢复](#452-usb-多层次恢复)
- [第5章 SSD 主控初始化与故障恢复](#第5章-ssd-主控初始化与故障恢复)
  ...
  - [5.3 NAND 通道初始化与介质探测](#53-nand-通道初始化与介质探测)
    - [5.3.1 NAND 通道初始化原理](#531-nand-通道初始化原理)
  ...
  - [5.8 SSD 异常检测与恢复](#58-ssd-异常检测与恢复)
    - [5.8.1 NAND 介质故障类型](#581-nand-介质故障类型)
    - [5.8.2 SSD 多层次恢复](#582-ssd-多层次恢复)
- [第6章 GPU 初始化与故障恢复](#第6章-gpu-初始化与故障恢复)
  - [6.1 GPU 供电与上电时序](#61-gpu-供电与上电时序)
  - [6.2 VBIOS / GPU 固件初始化](#62-vbios-gpu-固件初始化)
    - [6.2.1 VBIOS 功能与加载](#621-vbios-功能与加载)
    - [6.2.2 GPU 固件架构 (Modeset / PMU / SEC2)](#622-gpu-固件架构-modeset-pmu-sec2)
  - [6.3 显存初始化（HBM / GDDR 训练）](#63-显存初始化hbm-gddr-训练)
    - [6.3.1 HBM 初始化流程](#631-hbm-初始化流程)
    - [6.3.2 HBM 训练失败处理](#632-hbm-训练失败处理)
  ...
  - [6.7 GPU 异常检测与恢复](#67-gpu-异常检测与恢复)
    - [6.7.1 GPU XID 错误系统](#671-gpu-xid-错误系统)
    - [6.7.2 GPU 多层次恢复](#672-gpu-多层次恢复)
...
- [🛡️ RAS 综合设计手册](../../01_product/00_hardware/05_ras/2026-06-25-ras-comprehensive-handbook.md) — （RAS 理论基础）
- [🔌 PCIe 5.0 & 6.0 协议栈深度分析](../../01_product/00_hardware/01_hw-core/2026-06-25-pcie-protocol-stack-deep-analysis.md) — （LTSSM / AER / DPC）
- [🔌 SerDes 能力全景分析](../../01_product/00_hardware/04_si-signal/2026-06-25-serdes-comprehensive-analysis.md) — （PHY 初始化与均衡）
- 🛠️ 超节点可维护性与可观测性设计 — （固件管理）
- [⚡ DMA & RDMA 深度分析](../../01_product/00_hardware/01_hw-core/2026-06-25-dma-rdma-complete-analysis.md) — 关联
- [📡 信号质量分析体系](../../01_product/00_hardware/04_si-signal/2026-06-25-signal-integrity-analysis.md) — 关联
- 🎯 地址转换体系 — 关联
