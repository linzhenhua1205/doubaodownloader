...
- [一、训练 vs 推理：计算特性本质差异](#一训练-vs-推理计算特性本质差异)
  - [训练的核心矛盾：算力渴求型（Compute-bound）](#训练的核心矛盾算力渴求型compute-bound)
  - [推理的核心矛盾：存储带宽渴求型（Memory-bandwidth-bound）](#推理的核心矛盾存储带宽渴求型memory-bandwidth-bound)
  - [关键洞察](#关键洞察)
- [二、训练专用 GPU 设计建议](#二训练专用-gpu-设计建议)
  - [建议 ①：Matrix Engine 追求纯稠密算力](#建议-①matrix-engine-追求纯稠密算力)
  - [建议 ②：SRAM/L1 做大，减少 HBM 访问](#建议-②sraml1-做大减少-hbm-访问)
  - [建议 ③：互联带宽 > 互联范围](#建议-③互联带宽-互联范围)
  - [建议 ④：功耗设计](#建议-④功耗设计)
- [三、推理专用 GPU 设计建议](#三推理专用-gpu-设计建议)
  - [建议 ①：Memory bandwidth 是第一优先级](#建议-①memory-bandwidth-是第一优先级)
  - [建议 ②：KV Cache 专用硬件路径](#建议-②kv-cache-专用硬件路径)
  - [建议 ③：低精度推理链路硬件化](#建议-③低精度推理链路硬件化)
  - [建议 ④：PD 分离 × 芯片级支持](#建议-④pd-分离-芯片级支持)
  - [建议 ⑤：推理 GPU 的功耗/散热设计](#建议-⑤推理-gpu-的功耗散热设计)
- [四、MoE 对 GPU 设计的根本性冲击](#四moe-对-gpu-设计的根本性冲击)
  - [MoE 打破的两个"GPU 基本假设"](#moe-打破的两个gpu-基本假设)
  - [MoE 推理的"三元负载分解"](#moe-推理的三元负载分解)
- [五、MoE 训练专用 GPU 设计（arXiv 证据驱动）](#五moe-训练专用-gpu-设计arxiv-证据驱动)
  - [建议 ①：互联系统重新设计——从"越高越好"到"刚好够用+软件补足"](#建议-①互联系统重新设计从越高越好到刚好够用软件补足)
  - [建议 ②：NCCL EP 原生支持——硬件级 Expert Parallel 原语](#建议-②nccl-ep-原生支持硬件级-expert-parallel-原语)
  - [建议 ③：FoE 路线的硬件支持选项（预防性）](#建议-③foe-路线的硬件支持选项预防性)
  - [建议 ④：Expert Level 的容错硬件支持](#建议-④expert-level-的容错硬件支持)
- [六、MoE 推理专用 GPU 设计（arXiv 证据驱动）](#六moe-推理专用-gpu-设计arxiv-证据驱动)
  - [建议 ①：Attention-FFN 异构核心——专用硬件分解](#建议-①attention-ffn-异构核心专用硬件分解)
  - [建议 ②：KV Cache 专属硬件层级——MoE 的"第二内存瓶口"](#建议-②kv-cache-专属硬件层级moe-的第二内存瓶口)
  - [建议 ③：Expert 预取/缓存硬件——路由预测](#建议-③expert-预取缓存硬件路由预测)
  - [建议 ④：Token 粒度动态功率管理——MoE 的稀疏节能机会](#建议-④token-粒度动态功率管理moe-的稀疏节能机会)
- [七、qs 不等式对硬件路线的冲击](#七qs-不等式对硬件路线的冲击)
- [八、综合设计决策矩阵](#八综合设计决策矩阵)
- [九、最终建议清单（优先级排序）](#九最终建议清单优先级排序)
  - [🔴 P0：必须做（否则 MoE 推理效率极低）](#p0必须做否则-moe-推理效率极低)
  - [🟡 P1：强烈推荐（MoE 训练效率提升 20-50%）](#p1强烈推荐moe-训练效率提升-20-50)
  - [🟢 P2：具备前瞻性（应对路线不确定性）](#p2具备前瞻性应对路线不确定性)
- [附：关键引用论文清单](#附关键引用论文清单)
- [参考文件](#参考文件)
- [Changelog](#changelog)
...
1. **AlltoAll 延迟优化** — NVSwitch radix 提升，而非总带宽提升
2. **NCCL EP 硬件原语** — 支持 LL 模式的 ns 级 AlltoAll
3. **分层互联拓扑** — 域内全互联 + 域间 Rail 拓扑

### 🟢 P2：具备前瞻性（应对路线不确定性）

1. **核心级 DVFS** — 支持 Expert 粒度的功率管理，能效提升可达 26.3%（PALS）
2. **FoE 兼容性** — 互联拓扑支持 token-expert 亲和性映射
3. **支持 Dense ↔ MoE 双模式** — 不将任何一条路线堵死
...
> **关联文档**: [`gpu-ai-chips.md`](../../../01_survey/industry-research/01_topic-template/gpu-ai-chips.md)（竞争格局）· [`2026-06-26-moe-hardware-impact.md`](../../../03_AI/ai-principles/2026-06-26-moe-hardware-impact.md)（MoE 硬件影响跟踪）· [`server-form-factor.md`](../../../01_survey/industry-research/01_topic-template/server-form-factor.md)（服务器形态）· `server-hardware/`（硬件架构详情）
> **最后更新**: 2026-06-10
...
- [`gpu-ai-chips.md`](../../../01_survey/industry-research/01_topic-template/gpu-ai-chips.md) — 关联
- [`2026-06-26-moe-hardware-impact.md`](../../../03_AI/ai-principles/2026-06-26-moe-hardware-impact.md) — 关联
- [`server-form-factor.md`](../../../01_survey/industry-research/01_topic-template/server-form-factor.md) — 关联
