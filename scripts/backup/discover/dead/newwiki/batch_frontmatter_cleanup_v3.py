...
def clean_template_content(content):
    ...
    while i < len(lines):
        ...
        if state == 'before_h1':
            if line.startswith('# '):
                h1_title = line
                state = 'skip_template_section'
                i += 1
                continue
            i += 1
            
        elif state == 'skip_template_section':
            if line.startswith('## '):
                ...
                for name in template_section_names:
                    if section_title.startswith(name):
                        is_template_section = True
                        break
                
                if '问题总数' in section_title or '主题分布' in section_title:
                    is_template_section = True
            ...
            if re.match(r'^本专题聚焦', line):
                is_template_line = True
            
            if re.match(r'^\*\*主题分布\*\*', line):
                is_template_line = True
            
            if re.match(r'^\*\*核心要点\*\*', line):
                is_template_line = True
...
def process_file(filename, metadata):
    ...
    content = clean_template_content(content)
...
"""
    ...
    return original_len, new_len, char_count

results = []

for filename, metadata in sorted(file_metadata.items()):
    if os.path.exists(os.path.join(wiki_dir, filename)):
        try:
            orig_len, new_len, content_len = process_file(filename, metadata)
            results.append({
                "filename": filename,
            ...
            print(f"✅ {filename} - 处理完成 (移除 {orig_len - new_len:,} 字)")
...
print(f"共移除模板化内容: {total_removed:,} 字")

with open(os.path.join(wiki_dir, 'frontmatter_cleanup_v3_results.json'), 'w', encoding='utf-8') as f:
