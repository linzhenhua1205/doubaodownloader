...
def is_template_section_title(title):
    template_patterns = [
        r'^概述$',
        r'^简介$',
        r'^引言$',
...
def clean_template_content(content):
    ...
    while i < len(lines):
        ...
        if line.startswith('## '):
            ...
            if is_template_section_title(section_title):
                skip_section = True
                i += 1
                continue
            else:
...
with open(os.path.join(wiki_dir, 'frontmatter_cleanup_final_results.json'), 'w', encoding='utf-8') as f:
