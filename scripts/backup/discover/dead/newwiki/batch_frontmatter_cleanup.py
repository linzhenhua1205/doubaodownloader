...
def clean_template_content(content):
    ...
    found_first_real_section = False
    
    i = 0
    while i < len(lines):
        line = lines[i]
        
        if line.startswith('# ') and not found_first_real_section:
            found_first_real_section = True
            cleaned_lines.append(line)
            i += 1
            continue
        
        if not found_first_real_section:
            patterns_to_skip = [
                r'^>\s*\*\*定位\*\*',
                r'^>\s*\*\*问题总数\*\*',
                r'^>\s*\*\*构建时间\*\*',
            ...
            if line.strip() != '' or (len(cleaned_lines) > 0 and cleaned_lines[-1].strip() != ''):
                cleaned_lines.append(line)
            i += 1
        else:
            cleaned_lines.append(line)
...
with open(os.path.join(wiki_dir, 'frontmatter_cleanup_results.json'), 'w', encoding='utf-8') as f:
