...
def clean_template_content(content):
    ...
    while i < len(lines):
        ...
        if state == 'before_h1':
            if line.startswith('# '):
                h1_title = line
                state = 'after_h1_skip_template'
                i += 1
                continue
            result_lines.append(line)
            i += 1
            
        elif state == 'after_h1_skip_template':
            if line.startswith('## '):
...
with open(os.path.join(wiki_dir, 'frontmatter_cleanup_v2_results.json'), 'w', encoding='utf-8') as f:
