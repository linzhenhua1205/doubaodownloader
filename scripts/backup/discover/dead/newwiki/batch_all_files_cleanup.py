...
def clean_header_quotes(content):
    lines = content.split('\n')
    result_lines = []
    found_h1 = False
...
def process_file(filename, metadata):
    ...
    content = clean_header_quotes(content)
...
with open(os.path.join(wiki_dir, 'all_files_cleanup_results.json'), 'w', encoding='utf-8') as f:
